...
- **Battery optimization setup, surfaced in the app.** A drain now runs for minutes and can be held open by the coalescing window, so Doze or OEM background throttling silently killing the service is a live failure mode on a dedicated relay phone — and one that looks like "messages just stopped" rather than an error. Add a **Battery** section to Settings, following the exact pattern the `WRITE_SECURE_SETTINGS` notice already uses (4.0): show live status, and a tappable notice that copies the adb command to the clipboard.
  - Status: `PowerManager.isIgnoringBatteryOptimizations(getPackageName())` (API 23+), shown as exempt / not exempt.
  - Commands to offer for copying:
    - `adb shell dumpsys deviceidle whitelist +com.sh7411usa.jrelay` — Doze exemption.
    - `adb shell cmd appops set com.sh7411usa.jrelay RUN_ANY_IN_BACKGROUND allow` — background-execution throttling.
  - Because this app is sideloaded rather than Play-distributed, it may also declare `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` and offer an in-app button firing `ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`, so the host can grant it without a PC. Play policy restricts that permission; it does not apply here. Keep the adb path regardless, as the fallback.
  - Note in the section that some OEM skins (Xiaomi, Huawei, Oppo, Samsung) add proprietary "protected app" / "auto-start" settings that adb cannot reach and must be set by hand on the device. Say so plainly rather than implying the adb commands are sufficient everywhere.
- **Make the drain a foreground service.** This is the root-cause fix for a cluster of problems the battery-optimization and `BOOT_COMPLETED` items below only paper over, and it is arguably worth doing *before* Phase 3, since it protects what already ships in 5.1. A started service running a multi-minute drain is fragile under background execution limits by construction: Doze, OEM throttling and ordinary memory pressure can all kill it mid-fan-out, and the symptom is "messages stopped" with nothing in the UI to explain it. `startForeground` with a persistent notification takes the service out of that category entirely — and on a dedicated relay phone the notification is a feature, not a cost: it can show live queue depth and next-burst countdown, which is exactly what a host wants glanceable.
  - API 34+ (target is 36) requires `android:foregroundServiceType` on the `<service>` plus the matching permission. `shortService` is the wrong fit — it caps around 3 minutes, which a 100-member fan-out already brushes against. `specialUse` (with `FOREGROUND_SERVICE_SPECIAL_USE` and the `PROPERTY_SPECIAL_USE_FGS_SUBTYPE` manifest property) is the honest description; `dataSync` also works but is capped at 6 hours/day on API 35+. Play policy review does not apply — this app is sideloaded.
  - On API 31+ an app generally cannot start a foreground service from the background, but an app handling `SMS_RECEIVED` is on the temporary power allowlist and may. That is the existing entry point, so this fits without new triggers. The Phase 2 in-thread re-entry already avoids restarting the service mid-drain, so the FGS is started once and lives for the whole drain.
  - Interacts with Phase 6: a permanent notification is right for a dedicated device and wrong for someone's personal phone. If both ship, the foreground service should follow `RelayScope` — or at minimum use a minimum-importance notification channel.
- **No `BOOT_COMPLETED` receiver.** The manifest registers only `SMS_RECEIVED`, so after a reboot nothing resumes the outbox: queued messages sit until some member happens to text the group, which on a quiet night could be hours. This matters more now than in 4.1 — a drain can be holding rows for the coalescing window when the reboot lands — and it compounds the known "`SENDING` rows are never reset at startup" gap, since a reboot mid-drain both strands those rows and leaves nothing to kick the queue. It is also not hypothetical on the target hardware: Samsung ships **Auto restart at set times**, sometimes enabled by default.
  Add a `BOOT_COMPLETED` receiver (with the `RECEIVE_BOOT_COMPLETED` permission) that resets orphaned `SENDING` rows to `PENDING` and starts one drain. Pair it with the `SENDING`-reset-at-startup fix already logged from the Phase 2 review — they are the same failure and should land together.
