...
8. **Full fan-out with the app backgrounded** — watch logcat for `IllegalStateException` and for the service being stopped mid-drain. Closest thing to real operating conditions.
9. **Upgrade in place from the installed 5.0 build with messages already queued** — the backlog drains and those bodies go out unchanged. The 4→5 migration has never run against a real database.
10. **Pause pressed mid-fan-out** — sending stops within one burst.
11. **Coalesce window set to 0** — one post behaves as 5.0 did, allowing for the three documented deviations (per-recipient salting, pause honoured mid-drain, single-drainer guard).
12. **A post crossing the segment cap** — recipients get two texts, nothing lost or duplicated.

After 1, 3 and 6, confirm the dashboard queue count returns to 0 and no rows are left in `SENDING` — the cheapest check for the stranded-row risk under real conditions.
